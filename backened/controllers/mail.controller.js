  ...
  try {
    ...
    if (!email.verify) {
      return res.status(400).json({
        success: false,
        ok: false,
        message: "Please verify your email before reset pass!",
      });
    }
    ...
    var urlResetpass = `${request.protocol}://${request.get("host")}/?token=${tokenVerify}`;

    var content = "";
    content +=
      '<p style="font-size: 14px; line-height: 170%;">' +
      '<span style="font-size: 16px; line-height: 27.2px;">Hi ' +
      email.name +
      `,  </span></p>
      <p style="font-size: 14px; line-height: 170%;">
      ...
      <a href="` +
      urlResetpass +
      `" target="_blank" style="box-sizing: border-box;display: inline-block;font-family:arial,helvetica,sans-serif;
